  ...
  if (schedulesSnap.exists()) {
    schedulesSnap.forEach((child) => {
      ...
      const classIds = Array.isArray(row.classIds)
        ...
        : Object.values((row.classIds || {}) as Record<string, unknown>)
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const selection of selections) {
      ...
      if (!isScheduledForClass(prayerType, dayKey, dayOfWeek, student.className, config.schedules, config.overrides)) continue;
