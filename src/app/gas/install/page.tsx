...
export default function GasStudentInstallPage() {
  ...
  const safeVersionName = (apkMeta.versionName || "").replace(
    /[^0-9A-Za-z._-]/g,
    "_"
  );
  const safeVersionCode =
    typeof apkMeta.versionCode === "number" ? String(apkMeta.versionCode) : "";
  const downloadFileName =
    safeVersionName && safeVersionCode
      ? `GAS-Siswa-${safeVersionName}-${safeVersionCode}.apk`
      : apkMeta.fileName;
