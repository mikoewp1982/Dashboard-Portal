...
export function useGasLibrary(schoolId: string | undefined, selectedClass: string) {
  ...
  const addTask = async (task: Omit<LibraryTask, "id">) => {
    ...
    if (task.startAt !== undefined) rtdbPayload.startAt = task.startAt;
    if (task.endAt !== undefined) rtdbPayload.endAt = task.endAt;
    ...
    try {
      ...
      if (task.startAt !== undefined) fsPayload.startAt = task.startAt;
      if (task.endAt !== undefined) fsPayload.endAt = task.endAt;
