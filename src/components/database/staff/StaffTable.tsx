...
export function StaffTable({ rows, loading, onEdit, onDelete }: StaffTableProps) {
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900/40">
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center">
                  ...
                  <p className="font-medium text-slate-400">Belum ada data Petugas OSIS</p>
                </td>
              </tr>
            ) : (
              rows.map((row) => (
                <tr key={row.id} className="transition hover:bg-white/5">
                  ...
                  <td className="px-6 py-4 font-semibold text-slate-300">{row.nisn || row.id}</td>
      ...
      {!loading && (
        <div className="border-t border-white/10 bg-slate-900/20 px-6 py-4 text-xs text-slate-500">
          Menampilkan {rows.length} data petugas OSIS
        </div>
      )}
    </div>
  );
