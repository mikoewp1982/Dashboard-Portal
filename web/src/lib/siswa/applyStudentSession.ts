...
function mapAuthError(error: unknown): string {
  ...
  if (code === "auth/invalid-credential" || code === "auth/wrong-password") {
    return "NISN salah atau tidak cocok dengan password.";
  }
  if (code === "auth/user-disabled") {
    return "Akun siswa dinonaktifkan. Hubungi admin sekolah.";
  }
  if (code === "auth/too-many-requests") {
    return "Terlalu banyak percobaan login. Coba lagi beberapa menit.";
  }
  if (/Firebase:\s*Error\s*\((auth\/[^)]+)\)/i.test(message)) {
    return "Gagal autentikasi. Periksa NPSN/NISN atau coba lagi.";
  }
  return message;
}

export async function applyStudentAuthSession(options: {
  email?: string;
  password?: string;
  customToken?: string;
  ...
  throw new Error("Sesi login siswa tidak tersedia. Coba lagi.");
}
