...
- [ ] **Unduhan APK** — `/gas/install` (dan EduLock bila diubah) menunjuk versi Final yang disepakati.
  - **GAS Siswa Final lokal terbaru** = **1.0.92-siswa (23089)** rebuild terbaru **2026-08-30 14:53**. Fokus patch: **cleanup total rule usulan sekretaris** (hapus jalur pending verification UI & API terpisah; input **Wali Kelas** dan **Sekretaris Kelas** sama-sama final langsung tanpa jalur usulan). Tetap menyimpan audit trail sumber input. File final aktif lokal: `Apk Release/Final/GAS-Siswa-release.apk` dan `Apk Release/Final/GAS-Siswa-1.0.92-siswa-23089.apk`. SHA256 alias final lokal `D16CDD254843B121D850E3ED772A5A2E287A8DF7457321875B3DA515958A7F12`.
  - **GAS Siswa unduhan web lokal saat ini** sudah **1.0.92-siswa (23089)** hasil sinkron **2026-08-30 14:53** dan **SUDAH live commit** `81bdb467`. Manifest publik lokal menunjuk ke file `web/public/apk/GAS-Siswa-release.apk` dan `web/public/apk/GAS-Siswa-1.0.92-siswa-23089.apk` dengan SHA `D16CDD254843B121D850E3ED772A5A2E287A8DF7457321875B3DA515958A7F12`.
  ...
  - **EduLock Final saat ini** = **1.3.22 (48)** rebuild terbaru **2026-08-28 18:51**. Membawa patch keamanan `1.3.22` sebelumnya plus build kerja terbaru fitur **Temukan Perangkat** (alarm keras + ACK status ke admin), **patch fallback audio 2 lapis** (alarm + fallback `STREAM_MUSIC` + Vibrator), dan **hardening enforcement** untuk 2 bug lapangan yang baru lulus uji HP fisik: (1) internet mati total > 60 detik benar-benar mengunci, (2) `Accessibility OFF` yang diabaikan tidak lagi cuma popup berulang, tetapi tetap memaksa stay di jalur EduLock. File `Final` dan `web/public/apk` lokal **SUDAH di-push** bersama commit `81bdb467` sesi ini. SHA Final/public lokal alias `F6D6C3EEE4882266CB59BFFC60150BEB8A73B4F7D533BB972CA2D90D86ADEC34`.
  - **Deploy web admin sesi ini live di 2 tahap commit**:
...
- **Status retest terbaru:**
  ...
  - **catatan distribusi:** APK final bersih terbaru sudah dibuild ulang dan disalin ke folder `Apk Release/Final` serta `web/public/apk` lokal. **Push live sudah dilakukan bersama commit `81bdb467`** sesi ini (menunggu rollout Firebase App Hosting hijau).
