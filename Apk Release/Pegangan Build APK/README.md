...
- [ ] **Unduhan APK** — `/gas/install` (dan EduLock bila diubah) menunjuk versi Final yang disepakati.
  - **GAS Siswa Final lokal terbaru** = **1.0.83-siswa (23080)** rebuild uji cepat **2026-08-28 malam**. Fokus patch: gate login siswa sekarang melakukan **fast-path cek lokal EduLock** lebih dulu (`setup/accessibility/device admin/protection`) sehingga kasus aksesibilitas EduLock belum aktif tidak lagi tertahan lama di overlay `Memeriksa Proteksi EduLock` hanya karena masih menunggu fetch `attendance/schedules`, `attendance/holidays`, atau telemetry `active_devices`. File final aktif lokal: `Apk Release/Final/GAS-Siswa-release.apk` dan `Apk Release/Final/GAS-Siswa-1.0.83-siswa-23080.apk`. SHA256 alias final lokal `2A405CBA2DCA6551385614584B098A9AD81E26F8B7B8F2105B7BD98586E13F4A`.
