...
**Versi APK yang diasumsikan:** EduLock Siswa **1.3.24** (`versionCode` 50)  
**File Final:** `Apk Release/Final/EduLock-1.3.24-50.apk` + alias `EduLock-studentRelease.apk`  
**Sumber:** kode `native-mobile-edulock/` + dashboard web EduLock (bukan tebakan fitur)  
**Tanggal dokumen:** 2026-09-02 (Release v1.3.24 / 50)  

> **Catatan build terkini (v1.3.24 / 50):**
> 1. **Force Update Overlay Native & Bypass Browser (1.3.24 / 50):** Layar merah force update dialihkan 100% ke native `ForceUpdateActivity` (bukan floating overlay OS). Tombol *"DOWNLOAD UPDATE"* dan *"TUTUP APLIKASI"* responsif. Selama status force update aktif, sistem memberikan `FORCE_UPDATE_BYPASS` sehingga browser Chrome/Web dan Package Installer bebas dibuka tanpa ditendang gembok.
> 2. **Prosedur Unblock Layar Merah dari Super Admin (1.3.24):** Jika HP siswa terkunci layar merah force update, Super Admin cukup membuka menu **Kelola Versi Aplikasi** (`/super-admin/mobile-apps`), lalu menurunkan `min_version_code_edulock` ke **`1`** dan simpan. Layar merah di seluruh HP siswa akan otomatis tertutup dalam 1 detik.
> 3. **Relaksasi Aksesibilitas di Luar Jam Sekolah (1.3.23 / 49):** Di rumah / luar jam sekolah, dialog modal paksa *"Wajib Aktifkan Proteksi"* dan spam notifikasi 30 detik otomatis ditiadakan. HP siswa hening total di rumah.
> 4. **Fix Infinite Loop Pet Dead Lock (1.3.23 / 49):** Menghapus dialog sistem OS Android *"Penyematan Layar"* (`startLockTask`) pada hukuman pet mati. Tombol *"Saya Mengerti"* menutup layar dengan bersih sesuai siklus jeda admin (misal 5 menit -> 3 menit -> 1 menit).
> 5. **Device Admin Policy Hardening (1.3.23 / 49):** Menghapus tag berbahaya (`<wipe-data />`, `<reset-password />`, dll) dari `device_admin.xml`, hanya menyisakan `<force-lock />` dan `<disable-keyguard-features />`.
> 6. **Anti-Uninstall selektif 24/7 (1.3.19–1.3.22):** Device Admin / detail EduLock / dialog uninstall EduLock → ditendang (`BACK` + `HOME` + relaunch). Daftar aplikasi lain tetap boleh dikelola. Kecuali izin Uninstall Bypass (“Buka Gembok”) dari dashboard.
> 7. **Status unduhan web (`/e` dan `/edulock/install`):** Otomatis menyajikan `EduLock-1.3.24-50.apk`.

## UPDATE 2026-08-27 malam - Handoff QA USB E2E & PR bug belum final

### Ringkasan hasil uji hari ini

- Uji dilakukan **realtime di HP fisik via USB debugging** dengan web uji coba tenant sekolah demo.
- Device acuan memakai EduLock **1.3.22 (48)**.
- Jadwal masuk/pulang hari ini di admin sudah diperpanjang untuk kebutuhan simulasi E2E.
- Enforcement inti **lulus**:
  - device terbaca **di area sekolah**
  - jam sekolah aktif
  - proteksi aktif
  - percobaan buka app hiburan tetap ditahan EduLock
- Flow resmi **Minta Izin Guru** juga **lulus** end-to-end.

### Temuan utama yang masih terbuka

- Ada **keluarga bug recovery Settings**: saat proteksi inti tertentu dalam keadaan **OFF**, lalu admin mengubah proteksi sekolah dari **OFF -> ON**, overlay/prompt recovery muncul tetapi **jeda terlalu cepat** sehingga user tidak sempat menekan tombol untuk masuk ke halaman Settings yang dibutuhkan.
- Gejala ini **sudah terbukti nyata** pada:
  - **Accessibility OFF -> admin ON** = **masih gagal / belum final**
- Gejala ini **diduga satu akar** dan harus diperlakukan sebagai **satu PR bug** juga untuk:
  - **Tampil di atas aplikasi lain OFF -> admin ON**
  - **Izin Latar Belakang / Battery Optimization OFF -> admin ON**
  - **Izin Lokasi aplikasi OFF -> admin ON**
- Jalur **GPS mati -> buka Pengaturan Lokasi** sempat mereproduksi gejala serupa, tetapi pada retest terbaru **sudah normal lagi**.

### Build kandidat yang sempat dicoba malam ini

- Ada build kandidat internal hasil patch grace Settings yang dibuild ulang dari source dan disalin manual ke alias final:
...
- Metadata file kandidat internal:
  - timestamp: **2026-08-27 20:03:52**
  - size: **3.927.443 bytes**
- **Status:** jangan dianggap fix final, karena retest device fisik masih menunjukkan kasus **Accessibility OFF -> admin ON** belum terselesaikan.

## UPDATE 2026-08-27 larut malam - Progress lanjutan bug recovery Accessibility

### Ringkasan status terbaru

- PR bug keluarga recovery Settings **sudah bergerak maju signifikan**.
- Jalur pembanding **GPS mati -> buka Pengaturan Lokasi** tetap stabil / lulus.
- Jalur **`Accessibility OFF -> admin ON`** yang sebelumnya paling keras gagal, **sudah berhasil ditembus di HP fisik pada build kerja terakhir**.
- Namun, **status final clean build masih menunggu retest ulang** setelah instal dari file APK di folder `Apk Release/Final`, karena selama debugging sempat dipakai build kerja berinstrumen sebelum dibersihkan lagi.

### Akar teknis yang akhirnya paling relevan

1. [OverlayLockActivity.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/OverlayLockActivity.kt) untuk target recovery settings masih bisa **menutup dirinya sendiri terlalu cepat**:
   - `onResume()` mengecek `shouldStayLocked()` terlalu dini
   - untuk recovery `accessibility`, kondisi itu dapat bernilai `false` sebelum status proteksi benar-benar sinkron
   - akibatnya overlay recovery sempat muncul lalu `finish()` sendiri sebelum user sempat menekan tombol ke halaman Settings
2. [LockEnforcer.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/LockEnforcer.kt) memang perlu pola **debounce recovery overlay** ala fix GPS supaya overlay tidak dispam berkali-kali, tetapi sempat terbukti bahwa:
   - guard tambahan berbasis `lastForegroundPackage == settings` **terlalu agresif**
   - pada beberapa run, nilai foreground `settings` terbaca stale sehingga admin ON proteksi di HP justru terlihat seperti “tidak terjadi apa-apa”

### Perubahan final yang dipertahankan sampai progres ini

- Di [OverlayLockActivity.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/OverlayLockActivity.kt):
  - jalur recovery settings diprioritaskan dulu di `onResume()`
  - `shouldStayLocked()` menganggap recovery target aktif sebagai kondisi valid untuk tetap tampil
  - hasilnya: overlay recovery Accessibility tidak lagi `finish()` prematur
- Di [LockEnforcer.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/LockEnforcer.kt):
  - recovery target tetap diaktifkan dulu
  - `stopKiosk()` tetap dipanggil sebelum lempar overlay recovery
  - debounce overlay recovery **dipertahankan**
  - guard skip berbasis `lastForegroundPackage` **dicabut kembali** karena terbukti membuat false-negative (“proteksi admin saya ON tapi di HP tidak terjadi apa-apa”)

### Status QA sampai titik handoff ini

- **Lulus di HP fisik pada build kerja terakhir:**
  - `Accessibility OFF -> admin ON`
- **Sudah lulus lebih dulu dan tetap dianggap stabil:**
  - `GPS mati -> buka Pengaturan Lokasi`
- **Belum dikunci final / masih wajib retest setelah instal APK clean dari folder Final:**
  - `Accessibility OFF -> admin ON` pada APK final bersih
...
- APK final bersih tetap memakai **EduLock 1.3.22 (48)** tanpa bump versi.
- File final yang harus dipakai untuk retest clean:
  - `D:\Dashboard Portal\Apk Release\Final\EduLock-1.3.22-48.apk`
  - alias `D:\Dashboard Portal\Apk Release\Final\EduLock-studentRelease.apk`
- Sampai retest clean selesai, status dokumen ini adalah:
  - **bug utama sudah berhasil ditembus**
  - **belum boleh dianggap fix final rilis umum**

### Fokus source code untuk tim lanjutan

- [MainActivity.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/MainActivity.kt)
  - cek flow prompt `PROTEKSI WAJIB AKTIF`
  - cek pembukaan `ACTION_ACCESSIBILITY_SETTINGS`
  - cek reset agresif `isSettingsOpen` / `settingsGraceUntil` di `onResume`
- [MonitoringService.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/MonitoringService.kt)
  - cek enforcement yang masih memanggil relaunch/kiosk terlalu cepat saat recovery Settings sedang berlangsung
- [OverlayLockActivity.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/OverlayLockActivity.kt)
  - patch grace untuk target `accessibility` sudah pernah ditambahkan, tetapi di real device masih belum cukup
- [SetupActivity.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/SetupActivity.kt)
  - cek pola reset grace onboarding karena keluarga bug ini mirip dengan gejala di setup awal
- [LockEnforcer.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/LockEnforcer.kt)
  - cek apakah `relaunchEduLock()` + `requestKiosk()` perlu ditahan lebih lama saat flow recovery Settings aktif
- [PreferencesManager.kt](file:///D:/Dashboard%20Portal/native-mobile-edulock/app/src/main/java/com/sekolah/edulock/PreferencesManager.kt)
  - cek apakah perlu pembedaan state grace per target recovery, bukan satu flag global

### Hipotesis kerja yang paling masuk akal saat handoff

1. `settingsGraceUntil` saat ini **belum dihormati konsisten** di semua jalur enforcement.
2. `isSettingsOpen` / `settingsGraceUntil` kemungkinan **di-reset terlalu cepat** ketika app resume dari transisi ke Settings.
3. Satu grace global mungkin **tidak cukup** untuk membedakan recovery `Accessibility`, `Overlay`, `Battery`, dan `App Location Permission`.
4. Solusi final kemungkinan perlu:
   - state recovery yang lebih spesifik per target
   - penahanan `relaunchEduLock()` / `requestKiosk()` selama target Settings aktif
   - syarat clear-grace yang lebih ketat dan tidak hanya bergantung pada `onResume`

### Status rilis

- **Belum layak dirilis umum** sebagai fix final untuk PR ini.
- Jika tim ingin lanjut, gunakan entry log hari ini di [BUILD_LOG.md](file:///D:/Dashboard%20Portal/Apk%20Release/Pegangan%20Build%20APK/Edulock/BUILD_LOG.md) sebagai titik mulai, lalu retest minimal 4 skenario recovery di atas pada device fisik.
